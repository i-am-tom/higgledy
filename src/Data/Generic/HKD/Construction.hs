...
instance (Construct f inner, Applicative f)
    => GConstruct f (K1 index (Nested inner)) where
  gconstruct   (K1         x ) = fmap (K1 . Nested) (construct x)
  gdeconstruct (K1 (Nested x)) = K1 (deconstruct x)
